...
[![GitHub Super-Linter](https://github.com/akefirad/manual-approval-action/actions/workflows/linter.yml/badge.svg)](https://github.com/super-linter/super-linter)
![CI](https://github.com/akefirad/manual-approval-action/actions/workflows/ci.yml/badge.svg)
[![Check dist/](https://github.com/akefirad/manual-approval-action/actions/workflows/check-dist.yml/badge.svg)](https://github.com/akefirad/manual-approval-action/actions/workflows/check-dist.yml)
[![CodeQL](https://github.com/akefirad/manual-approval-action/actions/workflows/codeql-analysis.yml/badge.svg)](https://github.com/akefirad/manual-approval-action/actions/workflows/codeql-analysis.yml)
...
> [`nodenv`](https://github.com/nodenv/nodenv) or [`fnm`](https://github.com/Schniz/fnm), this
> template has a `.node-version` file at the root of the repository that can be used to
...
1. Format, test, and build the action
   ...
   > This step is important! It will run [`rollup`](https://rollupjs.org/) to build the final
   > JavaScript action code with all dependencies included. If you do not run this step, your action
   > will not work correctly when it is used in a workflow.

1. (Optional) Test your action locally

   The [`@github/local-action`](https://github.com/github/local-action) utility can be used to test
   your action locally. It is a simple command-line tool that "stubs" (or simulates) the GitHub
   Actions Toolkit. This way, you can run your TypeScript action locally without having to commit
   and push your changes to a repository.
   ...
   GitHub Actions Toolkit. For example, setting inputs and event payload data used by your action.
   For more information, see the example file, [`.env.example`](./.env.example), and the
...
[Actions tab](https://github.com/akefirad/manual-approval-action/actions)! :rocket:
...
steps:
  ...
  - name: Test Local Action
    id: test-action
    uses: akefirad/manual-approval-action@v1 # Commit with the `v1` tag
    with:
      milliseconds: 1000
...
non-compliant licenses. This workflow is initially disabled. To enable the workflow, follow the
below steps.
...
Once complete, this workflow will run any time a pull request is created or changes pushed directly
to `main`. If the workflow detects any dependencies with missing or non-compliant licenses, it will
fail the workflow and provide details on the issue(s) found.

### Updating Licenses

Whenever you install or update dependencies, you can use the Licensed CLI to update the licenses
database. To install Licensed, see the project's
